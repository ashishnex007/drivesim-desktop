...
class ScenarioRunner(object):
    ...
    def _load_and_wait_for_world(self, town, ego_vehicles=None):
        ...
        if self._args.sync:
            ...
            settings.fixed_delta_seconds = 0.06#1.0 / self.frame_rate
            self.world.apply_settings(settings)
    ...
    def _load_and_run_scenario(self, config):
        ...
        try:
            ...
            if self._args.spawn_vehicle:
                ...
                if self._args.spawn_vehicle_foreign: 
                    foreign_pat = 'foreign_vehicle'
                    spawn_points = self.spawn_specific_vehicle(foreign_pat, self._args.num_vehicles_foreign, tm, synchronous_master, spawn_points)
...
def main():
    ...
    parser.add_argument('--spawn_vehicle_foreign', action="store_true", help='Spawn Carla vehicles')
    parser.add_argument('--num_vehicles_foreign', metavar='W',
        default=5,
        type=int,
        help='Number of Nanos (default: 5)')
    ...
    # temporarily switching off sync mode
    if arguments.agent:
        arguments.sync = True
    arguments.sync = True
